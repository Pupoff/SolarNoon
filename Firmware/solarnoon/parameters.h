...
// ──  MIDI ─────────────────────────────────────────────────────────
// Section names (AMP, DELAY …) live in effectControls.h
// alongside the data they describe. Only the count is needed here for array sizes.

#define MAX_PARAMS 32  // max effectControls per section; needed for 2D array indexing
...
// Knob/Slider don't take a seed value: their initial 50 is just the soft-takeover
// crossing point until the DAW reports the real value (see GETTING_STARTED.md,
// "KNOB_MODE"), so one sane mid-range default suits every parameter equally.
#define PARAM_SEED_VALUE 50

// Switch/ThreeWay don't take one either: until the DAW reports the real state
// (`known` flips to true), the screen shows "?" regardless of this seed (see
// display.cpp), and 0 (off / first option) is a valid resting position for both.
#define SWITCH_SEED_VALUE 0

inline Parameter Knob    (int note, const char* label)                                 { return {PARAM_SEED_VALUE,  note, KNOB,             label, nullptr, false}; }
inline Parameter Slider  (int note, const char* label)                                 { return {PARAM_SEED_VALUE,  note, SLIDER,           label, nullptr, false}; }
inline Parameter Switch  (int note, const char* label)                                 { return {SWITCH_SEED_VALUE, note, SWITCH,           label, nullptr, false}; }
inline Parameter ThreeWay(int note, const char* label, const char* const* opts)        { return {SWITCH_SEED_VALUE, note, THREE_WAY_SWITCH, label, opts,    false}; }

// ── Global arrays (defined in effects.cpp) ────────────────────────────────────

extern Parameter effectControls[][MAX_PARAMS]; // first dim = SECTION_COUNT, defined in effectControls.h
extern int       effectControlCount[];
